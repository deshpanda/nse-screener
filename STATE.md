...
> DEAD (concentration REAL OOS: 19% of days = +241% vs +87% for the
> rest, then FADED this decade — decay law kill #7; trade dies at all
> cost tiers). QUEUE 2027 (fresh windows only): small-cap quality
> (external S&P600 profitability evidence; v31's floor was large-cap
> — but quality-family windows now spent), v4+profitability filter.
> Earlier today: CRISIS TEST (PROTOCOL_CRISIS, single shot,
...
> LAST ACT: v30 overnight anomaly (viral chart) — TRUE BUT
> UNTRADABLE, all predictions verified: real index OHLC shows
> overnight +1,691% / intraday −83% (2016-26), but C3 dies at every
> honest cost tier (0.10% RT loses BOTH windows; 0.05% loses IS);
> anomaly itself decaying (+836% OOS → +91% IS). TWO data traps
> struck & documented: bhav ETF OPEN prints unusable (±13% fake
> gaps, NIFTYBEES "overnight" compounds to +86e9% artifact) and the
> 70-row JSON cap hit the indicesHistory API (month-by-month fetch;
> data/indices/NIFTY_50_OHLC.parquet now exists, synced). Title now
> "Thirty Ways", counts 31/30 machine-checked. Earlier 07-17: v29.1
> combo registered (contaminated diagnostic disclosed — buys
> smoothness not return). Earlier 07-16: v29 GARP screen (viral Lynch tweet; PROTOCOL_V29,
> first VALUE-family test) — India: BEAT THE GATE BOTH WINDOWS
> (+53.6/+11.0, all 3 declared cells positive, best-month 11%) —
> registered prediction WRONG, printed. Incumbent rule on identical
> windows: v4 wins everything (+99.6/+118.9, Sharpe 1.21/1.63 vs
> 1.04/0.88) → 3rd gate-passer, 2nd "beat market, lost to v4".
> PRIZE: corr(v4)=0.54 IS / 0.39 OOS — lowest yet, best combo
> candidate found (future registration, v13.1 pattern, NOT now).
> US half: SEC EPS ingestion running (ingest/sec_fundamentals.py);
> US cells run when it lands, row updates then. Site: title now
> "Twenty-Nine Ways", counts 30/29 machine-checked. Earlier 07-15:
> v28 institutional accumulation DEAD — all 30 cells
...
> it same day: plateau 8/8 and tax PASS, but LOTTERY CHECK FAIL (74%
> of profit in one quarter — the v21 detector). Family closed. Site
> tells the whole three-act story in the v27 row + Season-2. Earlier:
> v27 synthetic version DEAD (n>=30 floor refused
...
| v29 | GARP screen from viral Lynch tweet (PROTOCOL_V29; PE<25, growth>15%, PEG<2, mcap floor; first value-family test; fwd-PE + D/E dropped, disclosed) | India: GATE PASSED both windows (+53.6 IS / +11.0 OOS; all 3 cells positive; framing debunked in protocol — $18M→$14B was AUM inflows) but LOSES to v4 on identical windows on every criterion (+99.6/+118.9, Sharpe 1.21/1.63 vs 1.04/0.88, half the DD) → incumbent stands. corr(v4) 0.54/0.39 = lowest yet → COMBO REGISTERED as v29.1 (PROTOCOL_V29.1, 2026-07-17): 70/30 A=v4/C=garp-primary; contaminated diagnostic disclosed (IS Sharpe 1.29 vs 1.21, OOS tie 1.63, total return LOWER both windows — buys smoothness not return); sleeve C paper-logged monthly to paper/garp.csv from Jul 31 (hook drilled, idempotent, never blocks main log); confirmation ONLY on future data: fidelity + corr<0.6 over ≥30 overlapping invested days + realized combo Sharpe ≥ realized v4 Sharpe, review ≥2027-04, ≥6 logged month-ends; cannot enter 2026 go-live; v13.1 retains seniority, any 3-sleeve question = new protocol. **US half: DEAD as predicted — primary −21.8 IS / −195.4 OOS 2010-22 vs SPY; SPY-regime variant worse (−53.3/−340.3), re-confirming v8's regime-hurts-in-US law independently. Same rules, opposite continents, opposite gate verdicts = edges-are-local part two. Data: ingest/sec_fundamentals.py (SEC XBRL datasets, 67 quarters, 48,752 TTM records, filed-date PIT) + us/data/raw_monthly.parquet (valuation needs unadjusted prices — the split-factor trap, same as India)** |
